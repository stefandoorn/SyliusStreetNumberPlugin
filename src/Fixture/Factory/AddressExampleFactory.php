...
class AddressExampleFactory extends AbstractExampleFactory
{
    ...
    protected function configureOptions(OptionsResolver $resolver): void
    {
        $resolver
            ...
            ->setDefault('number', function (Options $options): string {
                return $this->faker->buildingNumber;
            })
            ->setDefault('addition', function (Options $options): ?string {
                return random_int(1, 100) > 50 ? $this->faker->streetSuffix : null;
            })
    ...
    public function create(array $options = []): AddressInterface
    {
        ...
        $address->setNumber($options['number']);
        if (null !== $options['addition']) {
            $address->setAddition($options['addition']);
        }
